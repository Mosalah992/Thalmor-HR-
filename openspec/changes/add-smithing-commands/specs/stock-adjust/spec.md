...
and totals rows (column A matching `/TOTAL/i`). Row numbers from this read SHALL be used
directly as write targets so items added or moved by hand are handled without code changes.
...
The `item` option on all three commands SHALL offer autocomplete choices filtered from the
