...
Receive Discord interactions on a Cloudflare Worker: Ed25519 signature verification, command
routing, quartermaster-only authorization, and guild-scoped command registration — with no
always-on process, no channel IDs, and no Discord server-admin rights.
...
empty choice list. `/help` is exempt and available to everyone.

#### Scenario: Quartermaster invokes a command
- **WHEN** an allowlisted user runs `/add`, `/remove`, `/set`, or `/stock`
- **THEN** the command is processed normally

#### Scenario: Other member invokes a command
- **WHEN** a user not in `ALLOWED_USER_IDS` runs any ledger command
...
The registration script SHALL register the commands guild-scoped using only the bot token and
application ID, replacing the full command set idempotently (PUT). Registration MUST NOT require
channel IDs, channel permissions, or any Discord server setting, and MUST NOT set
`default_member_permissions` (which would hide the commands from the non-admin quartermaster).

#### Scenario: Re-running registration
- **WHEN** `node scripts/register-commands.js` is run twice
- **THEN** the guild ends up with exactly the defined command set, not duplicates
